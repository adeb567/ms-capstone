...
loc_n = "/home/amitabha/capstone/instance_version/FUnIE/resize_train_f/"
loc_e = "/home/amitabha/capstone/instance_version/FUnIE/enhanced_train_f/"

for image_path in listdir(loc_n):
    count += 1 
    loc = join(loc_n, image_path)
    nargin = 1
    normal.append(uciqe(nargin,loc))

for image_path in listdir(loc_e):
    loc = join(loc_e, image_path)
    nargin = 1
    enhanced.append(uciqe(nargin,loc))


print(mean(normal), mean(enhanced), count)

with open('some.csv', 'w') as f:
    writer = csv.writer(f)
    writer.writerows(zip(normal, enhanced))

plt.plot(normal, 'g', enhanced, 'r')
plt.show()
